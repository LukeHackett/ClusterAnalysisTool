...
{
  [TestClass]
  public class DropAnalysisTest : Test
  {
    #region Properties
    ...
    /// <summary>
    /// Product Analysis object to perform all the analysis
    /// </summary>
    private DropAnalysis Analysis;
    ...
    /// Analysis object will filter out all events that are not of type Drop.
    /// </summary>
    [TestMethod]
    public void TestAnalysisLength()
    {
      // Expected number of Drop events in the cluster
      int expected = Cluster.GetEvents("Drop").Count();

      // Actual number of drop events within the cluster
      int actual = Analysis.GetTotalDropCount();

      // Test to ensure the object has removed all non-drop events
    ...
    public void GroupByStartRatTest()
    {
      ...
      foreach(KeyValuePair<String, EventCollection> pair in results)
      {
        ...
        int expected = Cluster.GetEvents("Drop")
